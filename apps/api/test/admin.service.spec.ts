...
describe("AdminService ingestion workflow", () => {
  ...
  it("stores parsed JSON, confidence and status for manual email sources", async () => {
    ...
    const service = new AdminService(prisma as never, {} as never, parser as never, {} as never, { revalidate: jest.fn().mockResolvedValue(true) } as never, {} as never, uploadsStub as never);
  ...
  it("adds fetch warning when URL fetch fails", async () => {
    const result = parsedResult([candidate({ startsAt: "2099-07-04T18:00:00.000Z" })]);
    const prisma = {
