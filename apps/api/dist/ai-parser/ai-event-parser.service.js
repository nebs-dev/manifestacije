...
let AiEventParserService = class AiEventParserService {
    ...
    async parseBatchWithLlm(input) {
        ...
        if (this.isFacebookUrl(sourceUrl) && !input.screenshotBase64) {
            ...
            candidate.warnings.push("Facebook blokira automatsko dohvaćanje. Kopiraj tekst događanja s Facebook stranice i zalijepi ga u 'Ručni unos' s uključenim AI parserom, ili uploadaj screenshot.");
  ...
  "warnings": ["upozorenje1"]
        ...
        if (input.screenshotBase64) {
            userContent.push({
                ...
                source: {
                    type: "base64",
                    media_type: (input.screenshotMediaType ?? "image/jpeg"),
    ...
    extractHtmlImage(html, sourceUrl) {
    ...
    withFallbackImage(candidate, imageUrl, sourceUrl) {
    ...
    //   4.7., Event – City
    isVisitSlavoniaStyle(text) {
        const hasMonthHeading = /^(siječanj|veljača|ožujak|travanj|svibanj|lipanj|srpanj|kolovoz|rujan|listopad|studeni|prosinac)\s+20\d{2}/im.test(text);
        const hasInlineDate = /^\d{1,2}\.\d{1,2}\.,/m.test(text) ||
    ...
    parseVisitSlavoniaLines(text, sourceUrl) {
    ...
    parseMonthHeading(line) {
        const m = line.match(/^(siječanj|veljača|ožujak|travanj|svibanj|lipanj|srpanj|kolovoz|rujan|listopad|studeni|prosinac)\.?\s+(20\d{2})/i);
        if (!m)
            return null;
        const month = this.MONTH_NUMBERS[m[1].toLowerCase()] ?? 0;
        return month > 0 ? { month, year: parseInt(m[2], 10) } : null;
    }
    parseVisitSlavoniaLine(line, year, month, sourceUrl) {
        let rest = "";
        let startsAt = "";
        let endsAt = "";
        ...
        if (!city)
            warnings.push("Grad nije prepoznat – needs manual city");
    ...
    parseTitleAndLocation(text) {
        ...
        if (dashIdx >= 0) {
            ...
            if (knownCity)
                return { title, city: knownCity, description: afterDash };
    ...
    calcConfidence(missingFields, warnings, monthOnly = false) {
        let score = 0.9;
        ...
        const significantWarnings = warnings.filter((w) => !w.includes("nije u bazi") && !w.includes("Kategorija"));
    ...
    parseBlock(block, sourceUrl, organizerName) {
        ...
        const warnings = [];
        if (!date)
            warnings.push("Datum nije pronađen – needs manual date");
        if (!city)
            warnings.push("Grad nije prepoznat – needs manual city");
        if (!time)
            warnings.push("Vrijeme nije pronađeno, pretpostavljeno 18:00");
        if (city && !this.KNOWN_CITIES.some((c) => c.toLowerCase() === city.toLowerCase())) {
            warnings.push(`Grad '${city}' nije u bazi – may need to be added to taxonomy`);
        }
        return {
    ...
    isDateLine(line) {
        return (/^\d{1,2}\.\d{1,2}\.(20\d{2})/.test(line.trim()) ||
